...
while read line; do
    if [[ "$line" != *"NAME"* ]]; then
        ...
        helm get values --all --tls $release 1>"${HELM_DEPLOYMENT_DATA}/${release}_${chart}-values.out" 2>/dev/null
        [ $? -eq 0 ] || rm -f "${HELM_DEPLOYMENT_DATA}/${release}_${chart}-values.out"

        #grab history
        helm history $release --tls --col-width 5000 &> "${HELM_HISTORY_DATA}/${release}_${chart}.out"
